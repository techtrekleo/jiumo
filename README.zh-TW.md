...
現在墨裡只有一隻墨錦鯉在游。但我超想要一整個生態 —— 墨雀群、墨蝶飄、墨蛇、墨龍、墨水母…只要是能活在墨裡、會動的，什麼都好。

問題是 **我在美術方面的天份就跟答辯一樣**，叫我畫一個像樣的剪影簡直是酷刑。所以如果你會畫圖、或會寫一點 Canvas 2D，拜託來做幾隻生物。其實很簡單：

- **最簡單的路** —— 在 `app/engine/bodies.ts` 的 `BODY_PRESETS` 加一筆，寫個 `draw(ctx, S)` 畫出你那隻生物的**黑色剪影**就好。工作室會自動把它「印」進墨場，邊緣被流體拖成飄逸的墨絲。（直接照同一個檔裡的 `drawKoi` / `drawJellyfish` 改最快。）
- **進階** —— 想要像錦鯉那樣有脊椎、會擺動游動的，參考 `app/inklab/ink-creature.ts` 的 `Marrow`（骨架物理）+ `InkRenderer`（墨色暈染）。

你只要管「長什麼形狀」，墨的暈染、飄動、消散引擎全部幫你搞定。做好開個 PR，你會讓我超級開心 🖤
